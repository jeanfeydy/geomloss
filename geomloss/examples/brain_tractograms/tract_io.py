...
def streamlines_resample(streamlines,perc=None, npoints=None):
	...
	if perc is not None :
		resampled=[set_number_of_points(s,int(len(s)*perc/100.)) for s in streamlines ]
	else :
...
def check_ext(value):
	...
	if file_extension in ('.vtk','.xml','.vtp'):
		...
		raise argparse.ArgumentTypeError("Invalid file extension (file format supported: vtk,xml,vtp): %r" % value)
...
def check_resample(value):
	try:
		...
		if 0<=t<100:
			...
			raise argparse.ArgumentTypeError("Invalid resampling (must be between 0 and 100): %r" % value)
	except ValueError:
...
def setup():
	...
	parser.add_argument('Percentage',help='Resampling percentage value', type=check_resample)
