...
def sinkhorn_loop(
    *,
    ...
    This routine lets us compute the "biased" and "de-biased" Sinkhorn divergences
    ...
    for i, (eps, rho) in enumerate(zip(descent.eps_list, descent.rho_list)):
        ...
        # lists log_a_list, log_b_list, C_list, etc.
        #
        # The switch between different scales is specified by the list of "scale" indices,
        ...
        if i + 1 < len(descent.scale_list) and scale != descent.scale_list[i+1]:
            ...
            scale = next_scale
            log_a, log_b = log_a_list[scale], log_b_list[scale]
            C = C_fine
