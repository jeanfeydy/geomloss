...
def check_approx_equal(a, b, atol=1e-3):
...
def check_ot_result(us, gt, atol=1e-3):
    ...
    if gt.potential_a is not None:  # (Only if we expect to return the dual potentials)
        ...
        check_approx_equal(mean_us_a + mean_us_b, mean_gt_a + mean_gt_b, atol=atol)
        check_approx_equal(us_a - mean_us_a, gt_a - mean_gt_a, atol=atol)
        check_approx_equal(us_b - mean_us_b, gt_b - mean_gt_b, atol=atol)

    # Check that the two marginals are correct:
    check_approx_equal(us.marginal_a, gt.marginal_a, atol=atol)
